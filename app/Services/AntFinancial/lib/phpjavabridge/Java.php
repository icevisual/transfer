...
if (! function_exists("java_get_base")) {
    ...
    function java_autoload_function5($x)
    {
        ...
        if (! ($c->invokeMethod(0, "typeExists", array(
            $s
        ))))
            return false;
    ...
    function java_autoload_function($x)
    {
        ...
        if (! ($client->invokeMethod(0, "typeExists", array(
            $str
        ))))
            return false;
    ...
    class java_Client
    {
        ...
        function begin($name, $st)
        {
            ...
            switch ($name[0]) {
                case 'A':
                    ...
                    $newArg = new java_ApplyArg($this, 'A', $this->parser->getData($st['m']), $this->parser->getData($st['p']), $object, $this->getExact($st['n']));
        ...
        function cast($object, $type)
        {
            switch ($type[0]) {
                case 'S':
                case 's':
                    return $this->invokeMethod(0, "castToString", array(
                        $object
                    ));
                case 'B':
                case 'b':
                    return $this->invokeMethod(0, "castToBoolean", array(
                        $object
                    ));
                case 'L':
                case 'I':
                case 'l':
                case 'i':
                    return $this->invokeMethod(0, "castToExact", array(
                        $object
                    ));
                case 'D':
                case 'd':
                case 'F':
                case 'f':
                    return $this->invokeMethod(0, "castToInExact", array(
                        $object
                    ));
                case 'N':
                case 'n':
                ...
                case 'A':
                case 'a':
                    return $this->invokeMethod(0, "castToArray", array(
                        $object
                    ));
                case 'O':
                case 'o':
    ...
    class java_NativeParser
    {
        ...
        {
            $this->handler->protocol->handler->shutdownBrokenConnection(sprintf("protocol error: %s. Check the back end log for details.", $this->buf));
        }
    }
    ...
    class java_SimpleHttpHandler extends java_SocketHandler
    {
        ...
        function getWebAppInternal()
        {
            ...
            if (isset($context))
                return $context;
            return (JAVA_SERVLET == "User" && array_key_exists('PHP_SELF', $_SERVER) && array_key_exists('HTTP_HOST', $_SERVER)) ? $_SERVER['PHP_SELF'] . "javabridge" : null;
        }
            ...
            while (($str = trim(fgets($this->socket, JAVA_RECV_SIZE)))) {
                if ($str[0] == 'X') {
                    if (! strncasecmp("X_JAVABRIDGE_REDIRECT", $str, 21)) {
                        ...
                        if (! strncasecmp("X_JAVABRIDGE_CONTEXT", $str, 20)) {
                    ...
                    if ($str[0] == 'S') {
                        ...
                        if ($str[0] == 'C') {
                            if (! strncasecmp("CONTENT-LENGTH", $str, 14)) {
                                ...
                                if (! strncasecmp("CONNECTION", $str, 10) && ! strncasecmp("close", trim(substr($str, 11)), 5)) {
    ...
    class java_SimpleParser
    {
        ...
        function parserError()
        {
            $this->handler->protocol->handler->shutdownBrokenConnection(sprintf("protocol error: %s. Check the back end log for details.", $this->s));
        }
    }
    ...
    function java_values_internal($object)
    {
        if (! $object instanceof java_JavaType)
            return $object;
        $client = __javaproxy_Client_getClient();
        return $client->invokeMethod(0, "getValues", array(
            $object
        ));
    }
    ...
    function java_unwrap($object)
    {
        ...
        $client = __javaproxy_Client_getClient();
        return $client->globalRef->get($client->invokeMethod(0, "unwrapClosure", array(
            $object
        )));
    }
    ...
    function java_inspect_internal($object)
    {
        if (! $object instanceof java_JavaType)
            throw new java_IllegalArgumentException($object);
        $client = __javaproxy_Client_getClient();
        return $client->invokeMethod(0, "inspect", array(
            $object
        ));
    }
    ...
    {
        $client = __javaproxy_Client_getClient();
        return $client->invokeMethod(0, "setFileEncoding", array(
            $enc
        ));
    }
    ...
    function java_instanceof_internal($ob, $clazz)
    {
        ...
        $client = __javaproxy_Client_getClient();
        return $client->invokeMethod(0, "instanceOf", array(
            $ob,
            $clazz
        ));
    }
    ...
    class java_JavaProxy implements java_JavaType
    {
        ...
        {
            try {
                return $this->__client->invokeMethod(0, "ObjectToString", array(
                    $this
                ));
